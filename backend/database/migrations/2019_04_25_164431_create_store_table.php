...
class CreateStoreTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('store', function (Blueprint $table) {
            ...
            $table->string('remark', 100)->nullable(true)->comment('简介');
            /**
             * 物品类型
             * 1:卡片
             * 2:碎片
            ...
            $table->integer('exchange_crad_id')->nullable(true)->comment('交换卡片id，类型为卡片时填写');
            $table->tinyInteger('swop')->nullable(false)->comment('需要交换物品类型');
            $table->tinyInteger('swop_number')->nullable(false)->comment('需要交换物品数量');
            $table->integer('swop_crad_id')->nullable(true)->comment('需要交换卡片id，类型为卡片时填写');
            $table->tinyInteger('status')->default(0)->nullable(false)->comment('状态 0:下架 1:上架 2:交易成功');
            $table->integer('buyer_id')->nullable(true)->comment('购买人id');
            $table->timestamps();
